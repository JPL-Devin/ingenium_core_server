...
exports.create_environment_step = async function(args, res, next, headers) {
  /**
   * Create an environment step
   *
  ...
  try {
    const data = await node_funcs.createArchiveElement(execution_id, null, "STEP", "ENVIRONMENT_MANUAL", step, insert_after_id, level, key); 
...
exports.get_environment_step = async function(args, res, next, headers) {
  /**
   * Get an environment step
   *
...
exports.get_environment_step_input = async function(args, res, next, headers) {
  /**
   * Get user input of an environment step
   *
   * step_id String unique id of step
   * returns EnvironmentValueSpec
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.get_environment_step_result = async function(args, res, next, headers) {
  /**
   * Get result of an environment step
   *
   * step_id String unique id of step
   * returns EnvironmentResultSpec
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.get_execution_environment_steps = async function(args, res, next, headers) {
  /**
   * Get environment steps of the execution. List is sorted by the order in the execution.
   *
   * execution_id String unique id of execution
  ...
  let elem_type = 'STEP';
  let step_type = 'ENVIRONMENT_MANUAL'
...
exports.update_environment_step = async function(args, res, next, headers) {
  /**
   * Update an environment step
   *
   * step_id String unique id of step
   * step EnvironmentStep Definition of environment step
...
exports.update_environment_step_input = async function(args, res, next, headers) {
  /**
   * Update user input of an environment step
   *
   * step_id String unique id of step
   * user_input EnvironmentValueSpec User input values
   * no response value expected for this operation
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
