...
exports.create_wait_evr_step = async function(args, res, next, headers) {
  /**
   * Create a wait evr step
   *
   ...
   * returns AddWaitEVRStepResponse
   **/
  ...
  try {
    const data = await node_funcs.createArchiveElement(execution_id, null, "STEP", "WAIT_EVR", step, insert_after_id, level, key); 
...
exports.get_execution_wait_evr_steps = async function(args, res, next, headers) {
  /**
   * Get wait EVR steps of the execution. List is sorted by the order in the execution.
   *
   * execution_id String unique id of execution
...
exports.get_wait_evr_step = async function(args, res, next, headers) {
  /**
   * Get a wait evr step
   *
...
exports.get_wait_evr_step_input = async function(args, res, next, headers) {
  /**
   * Get user input of a wait evr step
   *
   * elem_id String unique id of a procedure element
   * returns WaitEVRInputSpec
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.get_wait_evr_step_result = async function(args, res, next, headers) {
  /**
   * Get result of a wait evr step
   *
   * elem_id String unique id of a procedure element
   * returns WaitEVRResultSpec
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.update_wait_evr_step = async function(args, res, next, headers) {
  /**
   * Update a wait evr step
   *
   * elem_id String unique id of a procedure element
   * step WaitEVRStep Definition of wait evr step
...
exports.update_wait_evr_step_input = async function(args, res, next, headers) {
  /**
   * Update user input of a wait evr step
   *
   * elem_id String unique id of a procedure element
   * user_input WaitEVRInputSpec User input values
   * no response value expected for this operation
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
