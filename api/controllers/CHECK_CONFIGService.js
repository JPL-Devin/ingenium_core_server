...
exports.create_check_config_step = async function(args, res, next, headers) {
  /**
   * Create a check configuration step
   *
   ...
   * returns AddCheckConfigStepResponse
   **/
  ...
  try {
    const data = await node_funcs.createArchiveElement(execution_id, null, "STEP", "VENUE_CONFIG_CHECK", step, insert_after_id, level, key); 
...
exports.get_check_config_step = async function(args, res, next, headers) {
  /**
   * Get a check configuration step
   *
   * elem_id String unique id of a procedure element
   * returns CheckConfigStep
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
...
exports.get_check_config_step_input = async function(args, res, next, headers) {
  /**
   * Get user input of a check config step
   *
   * elem_id String unique id of a procedure element
   * returns CheckConfigValueSpec
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.get_check_config_step_result = async function(args, res, next, headers) {
  /**
   * Get result of a check config step
   *
   * elem_id String unique id of a procedure element
   * returns CheckConfigResultSpec
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.get_execution_check_config_steps = async function(args, res, next, headers) {
  /**
   * Get check config steps of the execution. List is sorted by the order in the execution.
   *
   * execution_id String unique id of execution
  ...
  let elem_type = 'STEP';
  let step_type = 'VENUE_CONFIG_CHECK'
...
exports.update_check_config_step = async function(args, res, next, headers) {
  /**
   * Update a check config step
   *
   * elem_id String unique id of a procedure element
   * step CheckConfigStep Definition of check config step
...
exports.update_check_config_step_input = async function(args, res, next, headers) {
  /**
   * Update user input of a check config step
   *
   * elem_id String unique id of a procedure element
   * user_input CheckConfigValueSpec User input values
   * no response value expected for this operation
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
