...
exports.create_manual_verification_step = async function(args, res, next, headers) {
  /**
   * Create a manual verification step
   *
   ...
   * returns AddManualVerificationStepResponse
   **/
  ...
  try {
    const data = await node_funcs.createArchiveElement(execution_id, null, "STEP", "MANUAL_VERIFICATION", step, insert_after_id, level, key); 
...
exports.get_execution_manual_verification_steps = async function(args, res, next, headers) {
  /**
   * Get manual verification steps of the execution. List is sorted by the order in the execution.
   *
   * execution_id String unique id of execution
  ...
  let elem_type = 'STEP';
  let step_type = 'MANUAL_VERIFICATION'
...
exports.get_manual_verification_step = async function(args, res, next, headers) {
  /**
   * Get a manual verification step
   *
   * elem_id String unique id of a procedure element
   * returns ManualVerificationStep
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
...
exports.get_manual_verification_step_input = async function(args, res, next, headers) {
  /**
   * Get user input of a manual verification step
   *
   * elem_id String unique id of a procedure element
   * returns ManualVerificationStepInput
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.get_manual_verification_step_result = async function(args, res, next, headers) {
  /**
   * Get result of a manual verification step
   *
   * elem_id String unique id of a procedure element
   * returns ManualVerificationStepResult
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.update_manual_verification_step = async function(args, res, next, headers) {
  /**
   * Update a manual verification step
   *
   * elem_id String unique id of a procedure element
   * step ManualVerificationStep Definition of manual verification step
...
exports.update_manual_verification_step_input = async function(args, res, next, headers) {
  /**
   * Update user input of a manual verification step
   *
   * elem_id String unique id of a procedure element
   * user_input ManualVerificationStepInput User input values
   * no response value expected for this operation
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  ...
  try {
    if (!user_input.verified_by) {
      user_input.verified_by = node_funcs.parse_username(key);
    }
    if (!user_input.time_verified) {
      user_input.time_verified = new Date();
    }
