...
exports.create_vi_status_step = async function(args, res, next, headers) {
  /**
   * Create a verification item status step
   *
   ...
   * returns AddVIStatusStepResponse
   **/
  ...
  try {
    const data = await node_funcs.createArchiveElement(execution_id, null, "STEP", "VERIFICATION_ITEM_STATUS", step, insert_after_id, level, key); 
...
exports.get_execution_vi_status_steps = async function(args, res, next, headers) {
  /**
   * Get VI status steps of the execution. List is sorted by the order in the execution.
   *
   * execution_id String unique id of execution
  ...
  let elem_type = 'STEP';
  let step_type = 'VERIFICATION_ITEM_STATUS'
...
exports.get_vi_status_step = async function(args, res, next, headers) {
  /**
   * Get a VI status step
   *
   * elem_id String unique id of a procedure element
   * returns VIStatusStep
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
...
exports.update_vi_status_step = async function(args, res, next, headers) {
  /**
   * Update a VI status step
   *
   * elem_id String unique id of a procedure element
   * step VIStatusStep Definition of VI status step
...
exports.get_vi_status_step_input = async function(args, res, next, headers) {
  /**
   * Get a VI status step input
   *
   * elem_id String unique id of a procedure element
   * returns VIStatusStepInput
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  let elem_id = args['elem_id']['value'];
...
exports.update_vi_status_step_input = async function(args, res, next, headers) {
  /**
   * Update a VI status step input
   *
   * elem_id String unique id of a procedure element
   * user_input VIStatusStepInput Definition of VI status step input
   * no response value expected for this operation
   **/
  const key = node_funcs.get_auth_key(headers);
  let execution_id = args['execution_id']['value'];
  ...
  try {
    const data = await node_funcs.update_step_input_nonexecutable(execution_id, null, elem_id, user_input, key); 
    res.status(204).end();
  } catch (err) {
