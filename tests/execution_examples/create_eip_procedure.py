        ...
        for i in range(num_sections):
            ...
            for j in range(num_steps_per_section):
                ...
                if j % 2 == 0:
                    ...
                    with open('eip.png','rb') as file:         
                        files = {'file_content': file}

                        file_info = self.add_elem_file(procedure_url, step_id, files, "eip.png")

                        url = f'/file_server/{file_info["url"]}'
                        print(f'url:', url)           
                        self.update_step(procedure_url, StepTypes.MANUAL_EIP, step_id, 
                        {'title': 'Step {}-{}'.format(i+1, j+1), 'description': f'<p><img src="{url}" style="width: 1166px;" class="fr-fic fr-dib"></p>' + random_text(5, 80)})                    
...
def print_usage():
    print('USAGE: python create_eip_procedure.py server num_sections num_steps_per_section num_entries')
    print('Example: python create_eip_procedure.py https://ingenium-ci.cld.jpl.nasa.gov 100 10 50')
