    ...
    def run_procedure(self, venue_id, procedure_id, version, timeout):
        ...
        try:
            ...
            for elem in proc_section_elems:
                if elem['elem_type'] == 'STEP' and elem['step_type'] == 'ENVIRONMENT_MANUAL':
                    self.set_step_input(execution_url, StepTypes.ENVIRONMENT_MANUAL, elem['elem_id'], user_input_0)    
        ...
        with open(out_file_name, 'a+') as f:
            ...
            for i in range(1, num_runs+1):
                ...
                try:
                    ...
                    out_line = 'pid: {} i: {} now: {} time_taken: {} time_elapsed: {} num_successes: {} num_failures: {}\n'.format(pid, i, now, time_taken, time_elapsed, num_successes, num_failures) 
                    print(out_line)
                    f.write(out_line)
                    f.flush()
...
def print_usage():
    print('USAGE: python run_procedures.py create_venues num_venues')
    print('USAGE: python run_procedures.py create_procedure num_sections num_steps_per_section')
    print('USAGE: python run_procedures.py run_procedure venue_id procedure_id version timeout')
    print('USAGE: python run_procedures.py run_procedures procedure_id version num_runs timeout')
    print('USAGE: python run_procedures.py run_procedures_multi_procs procedure_id version num_runs timeout num_procs')
...
if __name__ == "__main__":    
    ...
    if sys.argv[1] == 'create_venues':
        num_venues = int(sys.argv[2])
        rps = RunProcedures()
        venue_ids = rps.create_venues(num_venues)
        print(venue_ids)
    elif sys.argv[1] == 'create_procedure':
    ...
    elif sys.argv[1] == 'run_procedures':
    ...
    elif sys.argv[1] == 'run_procedures_multi_procs':
        procedure_id = sys.argv[2]
        version = int(sys.argv[3]) 
        num_runs = int(sys.argv[4])   
        timeout = int(sys.argv[5])
